    ...
    if (url.pathname === "/api/movies/sync" && request.method === "POST") {
      ...
      try {
        ...
        if (env.TRACKER_DB && payload) {
          ...
          } else if (payload.venues && typeof payload.venues === "object") {
          ...
          } else if (payload.cityCode && Array.isArray(payload.movies)) {
...
async function fetchShowsForTracker(tracker, env) {
  ...
  for (const evCode of codes) {
    ...
    try {
      ...
      const movieTitle = MOVIES_CATALOG[evCode] || json.metadata?.analytics?.title || tracker.movieTitle || "Movie";
      const dates = [];
